...
void LQRController::initialize(std::string name, tf2_ros::Buffer* tf, costmap_2d::Costmap2DROS* costmap_ros) {
  if (!initialized_) {
    ...
    drive_pub_ = nh.advertise<ackermann_msgs::AckermannDriveStamped>("/drive", 10);  // ✅ 퍼블리셔 추가
...
bool LQRController::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
{
  ...
  drive_msg.drive.speed = cmd_vel.linear.x;
  drive_msg.drive.steering_angle = cmd_vel.angular.z;
